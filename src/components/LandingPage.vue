...
onUnmounted(() => {
  ...
  // Remove dark mode when leaving landing page
...
<template>
    ...
    <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-14 sm:h-16">
        <div class="flex items-center gap-2 sm:gap-6">
          <div class="flex items-center gap-1.5 sm:gap-2 group cursor-pointer" @click="scrollToSection('hero')">
            <div class="relative">
              ...
              <Icon icon="lucide:palette" class="w-5 sm:w-6 h-5 sm:h-6 text-blue-400 relative z-10 group-hover:scale-110 transition-transform duration-150" />
            </div>
            <span class="font-bold text-sm sm:text-lg bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent group-hover:scale-105 transition-transform duration-150">Spool Swatch</span>
          </div>

          <!-- GitHub Stats in Nav - Hidden on mobile -->
        ...
        <div class="flex items-center gap-0.5 sm:gap-1">
          <a
            ...
            class="group hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 bg-pink-500/10 backdrop-blur-md rounded-full border border-pink-400/30 hover:border-pink-400/60 hover:bg-pink-500/20 transition-all duration-150 hover:scale-105 mr-2"
          >
  ...
  <div class="min-h-screen bg-gradient-to-br from-[rgb(var(--background))] via-[rgb(var(--background))] to-[rgb(var(--background))]/90 relative">
    ...
    <div 
      class="fixed w-96 h-96 rounded-full opacity-20 blur-3xl pointer-events-none transition-all duration-150 ease-out z-0 hidden sm:block"
      :style="{
        left: mouseX - 192 + 'px',
        top: mouseY - 192 + 'px',
    ...
    <!-- Enhanced Animated Background Gradients with Parallax - Hidden on mobile for performance -->
    ...
    <!-- Hero Section -->
    <section id="hero" class="relative min-h-screen flex items-start sm:items-center justify-center px-4 sm:px-6 lg:px-8 overflow-hidden pt-20 sm:pt-24">
      <!-- Floating Color Blobs - Hidden on mobile -->
      <div class="absolute inset-0 pointer-events-none z-0 hidden sm:block">
        ...
        <!-- 3D Printer Animation (Experimental) - Hidden on mobile -->
        <div class="absolute bottom-10 right-10 opacity-10 animate-print-slow hidden md:block">
        ...
        <!-- Filament Spool Animations (Experimental) - Hidden on mobile -->
        <div class="absolute top-20 left-10 opacity-10 animate-roll-slow hidden md:block">
        ...
        <div class="absolute bottom-32 left-1/4 opacity-10 animate-roll-medium hidden md:block">
      ...
      <div 
        class="max-w-6xl mx-auto text-center transition-all duration-150 transform relative z-10 pt-4 sm:pt-0"
        :class="isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'"
      >
        <!-- Enhanced Logo/Icon Animation -->
        <div class="mb-4 sm:mb-8 inline-block animate-float group cursor-pointer" @click="enterApp">
          <div class="relative">
            <div class="absolute inset-0 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 rounded-full blur-3xl opacity-60 animate-pulse scale-125 group-hover:scale-150 transition-transform duration-200 hidden sm:block"></div>
            <div class="absolute inset-0 bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-500 rounded-full blur-2xl opacity-40 animate-pulse hidden sm:block" style="animation-delay: 1s;"></div>
            <div class="relative bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 p-4 sm:p-8 rounded-xl sm:rounded-3xl shadow-2xl transform group-hover:scale-110 group-hover:rotate-6 transition-all duration-200 hover:shadow-blue-500/50">
              <div class="absolute inset-0 bg-white/10 rounded-xl sm:rounded-3xl backdrop-blur-sm"></div>
              <Icon icon="lucide:palette" class="w-12 sm:w-24 h-12 sm:h-24 text-white relative z-10 group-hover:rotate-12 transition-transform duration-200" />
            </div>
            <!-- Glowing ring - hidden on mobile -->
            <div class="absolute inset-0 border-4 border-blue-500/30 rounded-xl sm:rounded-3xl group-hover:border-blue-400/50 transition-colors duration-200 animate-ping hidden sm:block" style="animation-duration: 3s;"></div>
          </div>
        </div>

        <!-- Enhanced Heading with multiple gradients -->
        <h1 class="text-2xl sm:text-5xl lg:text-7xl font-bold mb-4 sm:mb-6 relative leading-tight px-2">
          <span class="relative inline-block">
            <span class="relative bg-gradient-to-r from-blue-700 via-purple-700 to-pink-700 dark:from-blue-300 dark:via-purple-300 dark:to-pink-300 bg-clip-text text-transparent font-extrabold" style="filter: drop-shadow(0 1px 2px rgba(0,0,0,0.4));">
              {{ t('landing.hero.title') }}
            </span>
          </span>
        </h1>
        
        <p class="text-base sm:text-xl lg:text-2xl text-[rgb(var(--foreground))] mb-3 sm:mb-4 max-w-3xl mx-auto font-semibold leading-relaxed px-4">
          {{ t('landing.hero.subtitle') }}
        </p>
        
        <i18n-t keypath="landing.hero.description" tag="p" class="text-sm sm:text-base lg:text-lg text-[rgb(var(--foreground))]/70 mb-6 sm:mb-12 max-w-2xl mx-auto leading-relaxed px-4">
          <template #spoolman>
            <a href="https://github.com/Donkie/Spoolman/" target="_blank" class="text-blue-500 dark:text-blue-400 hover:text-blue-400 dark:hover:text-blue-300 underline decoration-2 underline-offset-4 transition-all hover:scale-105 inline-block font-medium">Spoolman</a>
          </template>
        </i18n-t>

        <!-- Enhanced Requirements Notice -->
        <div class="max-w-2xl mx-auto mb-8 sm:mb-12 relative group px-4">
          <div class="absolute inset-0 bg-gradient-to-r from-blue-500/20 via-purple-500/20 to-pink-500/20 rounded-xl sm:rounded-2xl blur-xl opacity-50 group-hover:opacity-75 transition-opacity duration-150 hidden sm:block"></div>
          <div class="relative bg-[rgb(var(--surface))]/80 dark:bg-gradient-to-br dark:from-blue-500/10 dark:via-purple-500/10 dark:to-pink-500/10 border border-blue-500/40 rounded-xl sm:rounded-2xl p-4 sm:p-6 backdrop-blur-md group-hover:border-blue-400/50 transition-all duration-150 hover:shadow-2xl hover:shadow-blue-500/20">
            <div class="flex items-start gap-3">
              <div class="relative flex-shrink-0 mt-0.5">
                <div class="absolute inset-0 bg-blue-400 rounded-full blur-md opacity-50 animate-pulse hidden sm:block"></div>
                <Icon icon="lucide:info" class="w-5 sm:w-6 h-5 sm:h-6 text-blue-500 dark:text-blue-400 relative z-10 group-hover:scale-110 transition-transform duration-150" />
              </div>
              <div class="text-xs sm:text-sm text-[rgb(var(--foreground))]/80 leading-relaxed text-left">
                <strong class="text-[rgb(var(--foreground))] block mb-1 sm:mb-2 text-sm sm:text-base font-semibold">{{ t('landing.hero.requiresTitle') }}</strong>
                <i18n-t keypath="landing.hero.requiresText" tag="span">
                  <template #spoolman>
                    <a href="https://github.com/Donkie/Spoolman/" target="_blank" class="text-blue-500 dark:text-blue-400 hover:text-blue-400 dark:hover:text-blue-300 underline decoration-2 underline-offset-2 transition-colors font-medium">Spoolman</a>
                  </template>
                </i18n-t>
              </div>
            </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center mb-8 flex-wrap px-4">
          <Button 
            @click="enterApp"
            size="lg"
            class="group relative overflow-hidden bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 hover:from-blue-600 hover:via-purple-600 hover:to-pink-600 text-white border-0 px-6 sm:px-10 py-5 sm:py-7 text-base sm:text-lg font-bold transition-all duration-200 hover:scale-110 hover:shadow-2xl hover:shadow-blue-500/50 hover:-translate-y-1 w-full sm:w-auto"
          >
            ...
            <span class="relative z-10 flex items-center justify-center gap-2 sm:gap-3">
              <Icon icon="lucide:rocket" class="w-5 sm:w-6 h-5 sm:h-6 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-150" />
          ...
          <Button
            ...
            size="lg"
            variant="outline"
            class="group relative px-6 sm:px-10 py-5 sm:py-7 text-base sm:text-lg font-semibold border-2 hover:bg-gradient-to-r hover:from-blue-500/10 hover:to-purple-500/10 transition-all duration-150 hover:scale-105 hover:border-blue-400/50 hover:shadow-xl backdrop-blur-sm w-full sm:w-auto"
          >
